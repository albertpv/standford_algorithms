...
#Init all global vars of each node to 0
def init():
    for i in range(1,N+1):
        visited[i]=0
        finish[i]=0
...
#Recursive unplementation of DFS on a certain node
def dfs(G, i):
    global t
    visited[i]=1
    leader[i]=s
...
if __name__ == "__main__":
    init()
    # grev is the reversed graph and G the graph
    g, grev=getG()
    dfs_loop(grev) #THE FIRST LOOP ON REVERSE GRAPH

    # construct new graph with the SCC and do DFS with the biggest leader value
    newGraph={}
    for i in range(1,N+1):
        temp=[]
